...
Like other products that work with sensitive data, such as HR systems, advanced analysis in Viva Insights is only available for certain users who should be trained to handle sensitive information. Refer to [Privacy](../../privacy/privacy.md) when using data generated by Viva Insights.
...
You need to be assigned the role of **Insights Analyst** in the Viva Insights web app to create the report. You also need to have the June 2022 version or newer of Power BI Desktop version installed. [Get Power BI](https://powerbi.microsoft.com/en-us/desktop/).

### Q2. Can I share the underlying report dataset with others in my organization?

To maintain data privacy, only employees assigned the role of **Insights Analyst** in the Viva Insights web app should have access to the underlying dataset in the Power BI report.
...
* Column names, including metric names. When you set up the query for your Power BI report, the Viva Insights web app displays metric names based on your location settings. However, when query results load into the Power BI template, metric names are translated back into English.
